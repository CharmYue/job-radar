...
**封面**:Hero shot(全屏看板,S/A 满屏) + 大字标题:

> **找工作两个月 0 面试,装了这个 AI 插件第 3 天约面 5 家 😭**
>
> Boss 用得我裂开 → AI 5 分钟筛出神仙工作

**标题(挑一个,A/B 测)**:
- 🔥 找工作姐妹救命!这个 Chrome 插件让我一周锁定 3 个神仙 offer
- 🔥 别再人肉刷 Boss 了!AI 5 分钟筛 300 个岗位,我已读不回率从 80% 降到 20%
- 🔥 投简历投到失眠?装这个插件,AI 帮你抓 HR 还活跃的真·岗位
- 🔥 0 封号风险的 Boss AI 筛选神器!我已经用它拿到 5 个面试邀请
...
```
家人们谁懂啊,找工作两个月投了 80 份简历 0 回音,我已经准备摆烂了 😭

直到我意识到 — 不是我的问题,是 Boss 的排序根本不为我服务。

它给我推:标题"AI 算法工程师" 点进去全是 Python 后端,HR 三周前最后活跃的死岗,创业公司天天 996 还美其名"弹性"…我每天人肉刷 3 小时,投出去的还是踩坑王。

😤 火大之后,周末熬夜搓了一个 Chrome 扩展,把这些累活全塞给 AI。结果:

📊 装上的第 3 天,我筛 180 个岗位用了 5 分钟,投了 8 家 A 级以上 + HR 今日活跃的,**当天就回了 5 家**,这周已经约了 3 个面试。

【它做对了什么】

✅ 我把简历 + 偏好告诉它一次(6 维星级:薪资/大厂/不加班/稳定/通勤/技术栈匹配),它替我读完整 JD,不被标题党骗

✅ 每条岗位带 HR 活跃度,绿色=今日活跃 / 橘色=几周前 — 我现在只投绿色的,**投递回复率从 5% 飙到 50%**

✅ S→A→B→C→Reject 五档自动分级,带评分理由 + 担忧点 + 招呼语 — 想投哪个看一眼就清楚

✅ 全屏看板(独立 tab),按分数/HR/公司排序 + 全文搜索 — 比 Boss 自己的 UI 好用 10 倍

✅ 一键导 CSV 进 Excel 二次筛(我自己的 SOP:HR 今日活跃 + A 级以上 + 不投北京户口卡)

✅ 可选微信推送,跑完手机直接收清单

【为啥不会封号】

很多自动化工具一调 Boss 接口就 code=37 拉黑。这个不一样 — **不主动发请求,只观察浏览器里 Boss 自家 JS 的返回**。等于让 Boss 主动给我喂数据。装在自己 Chrome 上,跟手动浏览一模一样,风控拿你没办法。

【成本】

✅ MIT 开源免费
✅ 用你自己的 AI key(推荐 DeepSeek,扫 100 个岗位成本 < ¥1)
✅ 7 个 AI 模型可选:DeepSeek / 通义千问 / 豆包 / MiniMax / 智谱 / OpenAI / Claude
✅ 数据完全本地,不上传任何东西
...
人肉刷 Boss 刷到失眠,投 80 份 0 回 → 装这个 Chrome 插件第 3 天**约面 5 家** 😭

做了啥:
- 把简历 + 偏好(6 维星级:薪资/大厂/不加班/稳定/通勤/技术栈)告诉它一次
- 关键词 + 城市,一键 🚀
- AI 替我读 300 条 JD,5 分钟出 S/A/B 分级

最爽的点:
🟢 HR 活跃度颜色标 — **只投绿色今日活跃的,投递回复率 5% → 50%**
📊 全屏看板独立 tab,按 HR 排序 + 全文搜索
📥 一键导 CSV,Excel 里二次筛
🛡 **0 封号风险**:不爬接口只观察浏览器自家数据
💰 用自己的 AI key,DeepSeek 扫 100 个岗位 < ¥1
🆓 MIT 开源,数据完全本地
...
```
两个月投 80 份 0 回音,做了个 Chrome 插件让 AI 替我筛 Boss,第 3 天约面 5 家 👀

不是简历问题,是 Boss 的排序不为我服务。

它给我推:标题党岗位、HR 三周没活跃的死岗、号称弹性其实 996 — 我每天人肉刷 3 小时还是踩坑。

于是搓了求职雷达:

▸ 简历 + 6 维偏好(薪资/大厂/不加班/稳定/通勤/技术栈)告诉它一次
▸ 关键词 + 城市,🚀 一键跑
▸ AI 读完整 JD,S→A→B→C→Reject 5 档分级,带评分理由 + 担忧点 + HR 活跃度 + 招呼语

3 个救命特性:

1️⃣ HR 活跃度颜色标识 — 只投绿色今日活跃的,**回复率 5% 飙到 50%**
2️⃣ 评分理由 — "JD 写'弹性工作'但福利全是加班补助,实际 996 风险高" 这种坑帮你避
3️⃣ 全屏看板 + CSV 导出 — 按 HR 活跃度 + 分数双排,Excel 二次筛

🛡 0 封号:不爬接口,只观察 Boss 自家 JS 返回 — 风控拿不到
💰 用你自己的 AI key,扫 100 个岗位 < ¥1
🆓 MIT 开源,本地数据不上传
...
> 给 `https://www.bilibili.com/video/BV15XLi6qECr/` 用的视频描述。B 站推荐**带时间戳章节**(算法会用来做"高能预警"和精确推送),所以列了 timestamps 占位 — 你按实际视频内容调一下。

### 短版(140 字以内,用作标题下方简介)

```
求职两个月投 80 份 0 回音?我做了个 AI Chrome 扩展,5 分钟筛完 Boss 上 300 个岗位,自动按 S/A/B 分档,带评分理由 + HR 活跃度 + 微信推送。开源免费,本地运行,不爬接口零封号风险。GitHub: CharmYue/job-radar
```

### 完整版(可以塞 1000+ 字,B 站简介上限很高)

```
👋 Hi,我是 @CharmYue,这是我自己做来用的 Boss 直聘 AI 筛选扩展「求职雷达」。

📌 它解决什么:
人肉刷 Boss 每天 300 条岗位 2-3 小时,投出去 80% 已读不回。原因:Boss 不会按"是否适合你"排序,它按平台利益排。这工具让 AI 替你扫一遍,带评分理由 + 担忧 + HR 活跃度 + 招呼语 — 投了就有回音。

🛠️ 怎么做的:
不爬 Boss 接口(很容易封号)。改为 Chrome MV3 扩展注入主世界,monkey-patch window.fetch,只观察 Boss 自家 JS 已经返回的数据 — 等于让 Boss 主动喂数据给你。所有处理本地完成,数据不出你的电脑。
- 7 个 AI 模型可选:DeepSeek / 通义 / 豆包 / MiniMax / 智谱 / OpenAI / Claude
- 简历 + 6 维偏好星级(薪资/大厂/不加班/稳定/通勤/技术栈)综合打分
- 全屏看板 + CSV 导 Excel + 微信推送(可选)
- IndexedDB 存数据,30 天自动留存

⏱️ 章节
00:00 痛点:找工作 2 个月投 80 份 0 回音
00:45 装扩展:git clone + chrome://extensions/ 加载
01:30 配画像:简历 / 6 维偏好 / AI key
03:00 配搜索:关键词 chip + 城市 + 抓取量
03:45 🚀 跑一轮:采集 → 打分 → 推送
05:30 看结果:卡片 / 表格 / 🖥 全屏看板
07:00 CSV 导 Excel 二次筛
07:45 手机 WxPusher 推送 + 直接跳 Boss App 聊 HR
09:00 升级 / 数据清理 / 闭环总结

🔗 链接
📦 GitHub(MIT 开源):github.com/CharmYue/job-radar
📖 完整文档 + WxPusher 配置教程:见 GitHub README
💬 评论区交流 / Issue 反馈

⚖️ 自己电脑、自己账号、本地数据 — 不是爬虫,Boss 自家 JS 怎么发的就怎么用,投递还是要自己手动点。MIT 开源,数据不上传。

#求职 #boss直聘 #AI #ChromeExtension #开源 #秋招 #社招 #程序员 #校招 #拿offer #面试 #AI工具
```

### B 站 自定义标签(发布时填的 tag 框,最多 12 个)

```
求职  Boss直聘  AI工具  Chrome插件  开源  程序员  秋招  社招  找工作  自动化  DeepSeek  MV3扩展
```

> 💡 B 站算法看标签很重,优先选**精确**+**有流量**的:`求职` `Boss直聘` `AI工具` `开源` 是基础组合,`Chrome插件` / `MV3扩展` 引技术圈,`秋招` / `社招` / `找工作` 接情绪流。

### 视频置顶评论(自评)

```
🎬 0:00 痛点 / 0:45 装 / 1:30 配画像 / 3:00 配搜索 / 3:45 跑 / 5:30 看结果 / 7:00 CSV / 7:45 手机推送 / 9:00 收尾

📦 GitHub: github.com/CharmYue/job-radar
⚠️ 自己账号自用,投递还是要手动 — 不是爬虫,不替代你聊 HR

有问题评论里聊,Issues 上提 bug。
```
